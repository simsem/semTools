## Title: Copy each aspect of the lavaan object into a clipboard
## Author: Sunthud Pornprasertmanit
# Description: Copy each aspect of the lavaan object into a clipboard

# Copy: copy each aspect of the lavaan object into a clipboard; this function will be compatible with the inspect function in lavaan
copy <- function(object, what="summary", ...) {
	...
	if(what == "summary") {
		...
		write.table(miPowerFit(object, ...), file="clipboard", sep="\t", row.names=FALSE, col.names=TRUE)
	} else {
		target <- inspect(object, what=what)
		if(is(target, "lavaan.data.frame") || is(target, "data.frame")) {
			write.table(target, file="clipboard", sep="\t", row.names=FALSE, col.names=TRUE)
			...
			if(is(target[[1]], "list")) {
				...
				write.table(target[-1,], file="clipboard", sep="\t", row.names=FALSE, col.names=FALSE)
			} else {
				target <- listToDataFrame(target)
				write.table(target, file="clipboard", sep="\t", row.names=FALSE, col.names=FALSE)
			}
		} else {
			write.table(target, file="clipboard", sep="\t", row.names=TRUE, col.names=TRUE)
		}
	}
}
...
copySummary <- function(object) {
	...
	write.table(rbind(set1, set2, set3), file="clipboard", sep="\t", row.names=FALSE, col.names=FALSE)
}
