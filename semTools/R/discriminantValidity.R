...
#' function.
#' @param cutoff A cutoff to be used in the constrainded models in likelihood 
#' ratio tests.
...
discriminantValidity <- function(object, cutoff = .9, merge = FALSE, level = .95){
  ...
  psi <- lavaan::lavInspect(object,"free")$psi[lvs,lvs]
  ...
  ret <-  lavaan::parameterEstimates(object,ci =  TRUE, level = level) [covIndices, c("lhs", "op", "rhs", "est","ci.lower","ci.upper")]
  
  rownames(ret) <- seq_len(nrow(ret))
  
  
  # Add the chi test to all correlation pairs
  ...
  constrainedModels <- lapply(covIndices,function(i){
    ...
    if(merge){
      ...
      if(abs(pt$est[i]) > cutoff) thisCutoff <- pt$est[i]
      else thisCutoff <- ifelse(pt$est[i] <0, - cutoff, cutoff)
