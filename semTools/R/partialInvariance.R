...
partialInvariance <- function(fit, type, free = NULL, fix = NULL, return.fit = FALSE) { 
	...
	if(type %in% c("metric", "weak", "loading", "loadings")) {
		...
		if("means" %in% names(fit)) {
			...
			if("strict" %in% names(fit)) {
				fit1 <- fit$strict			
			} else if ("strong" %in% names(fit)) {
				fit1 <- fit$strong			
			} else {
				stop("Either strong or strict invariance models is needed in the 'fit' argument")
			}
		} else {
			stop("Strict invariance models is needed in the 'fit' argument")
		}
	} else {
	...
	if(numType == 1) {
		...
		fixCon <- matrix(NA, length(varfree), 2)
		colnames(fixCon) <- c("fix.chi", "fix.p")
		freeCon <- matrix(NA, length(varfree), 2)
		...
		if(!is.null(fix)) {
			for(i in seq_along(index)) {
				...
				if(!is(tryresult0, "try-error")) {
					...
					if(!is(compresult0, "try-error"))  freeCon[i,] <- unlist(modelcomp0[2,c(5, 7)])
				}
			...
			for(i in seq_along(index)) {
				...
				if(length(oldmarker) > 0 && oldmarker == varfree[i]) {
					...
					temp <- freeParTable(pt1, facinvarfree[i], "=~", varfree[i], 1:ngroups)
					temp <- constrainParTable(temp, facinvarfree[i], "=~", varfree[i], 1:ngroups)
					temp <- fixParTable(temp, facinvarfree[i], "=~", candidatemarker, 1:ngroups)
					newparent <- freeParTable(pt1, facinvarfree[i], "=~", varfree[i], 1:ngroups)
