...
## NEW PROCEDURE
## -------------
## (1) function ONLY to permute data
## (2) function(s) ONLY to save output from (permuted) data: AFI(s) and max(MIs)
## (3) fit model to list of data sets with lavaanList, include extra in FUN
## (4) methods extract from list (summary, show, hist)


## (3)
cl <- makePSOCKcluster(3)
parList <- parLapply(cl, dataList, function(x, model, fit) {
  lavaan::update(fit, model, data = x)
}, fit = fit, model = eval(fit@call$model))
stopCluster(cl)


## (1)
permuteData <- function(con, modelType = c("mgcfa","mimic"), covariates = NULL,
                        maxSparse = 10L) {
  G <- lavaan::lavInspect(con, "group")
  gLabels <- lavaan::lavInspect(con, "group.label")
  ...
  if (length(G)) {
    gData <- mapply(FUN = function(x, g, n) {
      y <- data.frame(as.data.frame(x), g, stringsAsFactors = FALSE)
      names(y) <- c(n, G)
      y
    }, SIMPLIFY = FALSE,
    x = lavaan::lavInspect(con, "data"), g = gLabels,
    n = lavaan::lavNames(con, type = "ov", group = seq_along(gLabels)))
    Data <- do.call(rbind, gData)
  } else {
    Data <- as.data.frame(lavaan::lavInspect(con, "data"))
    names(Data) <- lavaan::lavNames(con, type = "ov")
  }
  ## permute data
  if (modelType == "mgcfa") {
    nSparse <- 0L
    while (nSparse <= maxSparse) {
      ## permute grouping variable
      Data[ , G] <- sample(Data[ , G])
      ## for ordered indicators, check that groups have same observed categories
      ordVars <- lavaan::lavNames(con, type = "ov.ord")
      if (length(ordVars) > 0) {
        try(onewayTables <- lavaan::lavTables(Data, dimension = 1L, group = G,
                                              categorical = ordVars), silent = TRUE)
        ...
        ## After maxSparse tries, return NULL with warning
        if (nSparse == maxSparse) {
          warning(paste("After", maxSparse, "attempted permutations, no",
                        "permuted data sets had the same number of categories",
                        "in each group for all ordered indicators.  You can",
                        "try a larger 'maxSparse', larger 'nPermute', or",
                        "collapsing any categories with very few observations."))
          return(NULL)
        }
      }
      ## If you get this far, you can break WHILE loop
      break
    }
  }
  if (modelType == "mimic") {
    ## check that covariates are actual variables
    if (length(covariates) && !all(covariates %in% names(Data)))
      stop('These specified covariates are not columns in the data.frame:\n',
           paste(setdiff(covariates, names(Data)), collapse = ", "))
    ## permute covariates, separately in each group
    if (length(G)) {
      for (gg in gLabels) {
        gData <- Data[ Data[[G]] == gg, ]
        N <- nrow(gData)
        newData <- gData[sample(1:N, N), covariates, drop = FALSE]
        for (COV in covariates) Data[Data[[G]] == gg, COV] <- newData[ , COV]
      }
    } else {
      N <- nrow(Data)
      newData <- Data[sample(1:N, N), covariates, drop = FALSE]
      for (COV in covariates) Data[ , COV] <- newData[ , COV]
    }
  }
  Data
}
...
setClass("permuteMeasEq", slots = c(PT = "data.frame",
                                    ...
                                    #n.Sparse = "vector", ## FIXME: remove from summary
                                    oldSeed = "integer"))
...
getAFIs <- function(con, uncon = NULL, null = NULL, AFIs = NULL, moreAFIs = NULL) {
  AFI1 <- list()
  AFI0 <- list()
  leastSq <- grepl("LS", lavaan::lavInspect(con, "options")$estimator)
  ...
  if (!is.null(AFIs)) {
    ...
    if (!is.null(uncon))
      AFI1[[1]] <- lavaan::fitMeasures(uncon, fit.measures = AFIs,
                                       baseline.model = null)
    AFI0[[1]] <- lavaan::fitMeasures(con, fit.measures = AFIs,
                                     baseline.model = null)
  }
...
getMIs <- function(con, modelType = c("mgcfa","mimic"), param = NULL) {
  if (modelType == "mgcfa") {
    ## save all estimates from constrained model
    PT <- lavaan::parTable(con)[ , c("lhs","op","rhs","group","plabel")]
    ## extract parameters of interest
...
## (hidden?) function to extract all data, including optional extra output
getOutput <- function() {
...
permuteMeasEq <- function(nPermute, modelType = c("mgcfa","mimic"),
  ...
  if (showProgress) {
	...
	#pmcl <- function(...) { parallel::mclapply(...) } # Sunthud's idea
